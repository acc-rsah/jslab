...
function displayResume(index) {
    ...
    const htmlContent = `
        <h1>${resume.basics.name} (${resume.basics.AppliedFor})</h1>
        <p>Email: ${resume.basics.email}</p>
        <p>Phone: ${resume.basics.phone}</p>
        <p>Address: ${resume.basics.location.address}, ${
        resume.basics.location.city
    }, ${resume.basics.location.state}, ${resume.basics.location.postalCode}</p>
        <p>LinkedIn: <a href="${resume.basics.profiles.url}">${
        resume.basics.profiles.url
    }</a></p>
        <h3>Skills</h3>
        <p>${resume.skills.keywords.join(", ")}</p>
    `;
    document.getElementById("resumeDetails").innerHTML = htmlContent;
    updateNavigation();
}
...
function updateNavigation() {
    ...
    document.getElementById("nextButton").style.visibility =
        currentIndex < resumes.length - 1 ? "visible" : "hidden";
}

function filterResumes() {
    const jobTitle = document.getElementById("searchInput").value.toLowerCase();
    const filtered = resumes.filter((resume) =>
        resume.basics.AppliedFor.toLowerCase().includes(jobTitle)
    );
    ...
    if (filtered.length === 0) {
        ...
        resumes.splice(0, resumes.length, ...filtered);
        displayResume(0);
    }
}
